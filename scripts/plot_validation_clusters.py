...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create per-device validation scatter plots for compute and d2h_transfer times by batch index."
    )
    parser.add_argument(
        "-i",
        "--input",
    ...
    parser.add_argument(
        "-o",
        "--outdir",
        default="scripts/plots/validation_clusters",
    ...
    if not grouped:
        raise SystemExit("No matching CUDA/OpenCL compute or d2h_transfer rows found in input CSV.")
    ...
    for device in devices:
        ...
        fig.tight_layout()
        output_path = outdir / f"{slugify(device)}_validation_clusters.png"
